...
def criar_graficos_comparativos():
    ...
    dados_tempo = [python_bubble_numba_tempos, python_quick_tempos, go_bubble_tempos, go_quick_tempos]
    dados_memoria = [python_bubble_numba_memorias, python_quick_memorias, go_bubble_memorias, go_quick_memorias]
...
python_bubble_numba_tempos = []
python_bubble_numba_memorias = []
python_quick_tempos = []
...
if escolha == 's' or escolha == 'sim':
    ...
    if dados_salvos:
        python_bubble_numba_tempos = dados_salvos.get("python_bubble_numba_tempos", [])
        python_bubble_numba_memorias = dados_salvos.get("python_bubble_numba_memorias", [])
...
if escolha != 's' and escolha != 'sim':
    ...
    # Executar Python Bubble Sort Numba 10 vezes
    print("\nPython Bubble Sort (Numba):")
    for i in range(10):
        tempo, memoria = executar_comando("py python_bubble_sort_numba.py")
        if tempo is not None:
            python_bubble_numba_tempos.append(tempo)
            python_bubble_numba_memorias.append(memoria)
            print(f"  {i+1}: {tempo:.4f}ms, {memoria:.4f}KB")
    ...
    dados_para_salvar = {
        "python_bubble_numba_tempos": python_bubble_numba_tempos,
        "python_bubble_numba_memorias": python_bubble_numba_memorias,
...
print(f"python_bubble_numba_tempos = {python_bubble_numba_tempos}")
print(f"python_bubble_numba_memorias = {python_bubble_numba_memorias}")
print()
...
print(f"\n=== MEDIAS ===")
if python_bubble_numba_tempos:
    print(f"Python Bubble Numba - Média: {sum(python_bubble_numba_tempos)/len(python_bubble_numba_tempos):.4f}ms")
